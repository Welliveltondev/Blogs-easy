...
const BlogTable = ({blog,fetchBlogs,index}) => {
  ...
  return (
    <tr className='border-y border-gray-300'> 
      ...
      <td className='px-2 py-4 max-sm:hiden'>{BlogDate.toDateString()}</td>
      <td className='px-2 py-4 max-sm:hidden'> 
        <p className={`${blog.isPublished ? 'text-green-600' : 'text-orange-700'}`}>
          {blog.isPublished ? 'Published' : 'Unpiblished'}
        </p>
      </td>
  
      <td className='px-2 py-4 flex text-xs gap-3'>
        <button className='border px-2 py-0.5 mt-1 rounded cursor-pointer'>{blog.isPublished ? 'Unpublish' : 'Publish'}</button>
        <img className='w-8 hover:scale-110 transition-all cursor-pointer' 
